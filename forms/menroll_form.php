...
class menroll_form extends moodleform
{
    protected function definition()
    {
        ...
        // Enrollment duration!
        $durationoptions = ['0' => get_string('unlimited', 'local_multiple_enrollments')] + array_combine(range(1, 365), range(1, 365));
        $mform->addElement('select', 'enrol_duration', get_string('enrolduration', 'local_multiple_enrollments'), $durationoptions);
        $mform->setType('enrol_duration', PARAM_INT);

        // Enrollment type radio buttons!
        $radioarray = [];
        $radioarray[] = $mform->createElement('radio', 'enrollment', '', get_string('newenrollment', 'local_multiple_enrollments'), 'newenrollment');
        $radioarray[] = $mform->createElement('radio', 'enrollment', '', get_string('existingenrollment', 'local_multiple_enrollments'), 'existingenrollment');
        ...
        // User selection!
        $userselect = $mform->createElement('autocomplete', 'new_selecteduser', get_string('roleuser', 'local_multiple_enrollments'), $users, $useroptions);

        // Course selection!
        $courseselect = $mform->createElement('autocomplete', 'selectedcourse', get_string('coursesrole', 'local_multiple_enrollments'), $courses, $courseoptions);

        // Group the elements into a single row!
        $mform->addGroup([$userselect, $courseselect], 'courseusergroup', get_string('courseusergroup', 'local_multiple_enrollments'), ' ', false);
        ...
        $mform->addElement('autocomplete', 'existing_selecteduser', get_string('selectuser', 'local_multiple_enrollments'), $users, $options);
        ...
        $existingcoursesselect = $mform->createElement('select', 'existingcourses[]', get_string('existingcourse', 'local_multiple_enrollments'), $existingcourses, $selectattributes);
        ...
        $potentialcoursesselect = $mform->createElement('select', 'potentialcourses[]', get_string('potentialcourses', 'local_multiple_enrollments'), $potentialcourses, $selectattributes);
